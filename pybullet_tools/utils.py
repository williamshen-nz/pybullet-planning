...
DRAKE_IIWA_URDF = join_paths(
    MODEL_DIRECTORY, "drake/iiwa_description/urdf/iiwa14_polytope_collision.urdf"
)
WSG_50_URDF = join_paths(
    MODEL_DIRECTORY, "drake/wsg_50_description/urdf/wsg_50_mesh_visual.urdf"
...
PANDA_ARM_URDF = join_paths(
    MODEL_DIRECTORY, "franka_description/robots/panda_arm_hand.urdf"
)
...
# Objects
SMALL_BLOCK_URDF = join_paths(
    MODEL_DIRECTORY, "drake/objects/block_for_pick_and_place.urdf"
)
BLOCK_URDF = join_paths(
    MODEL_DIRECTORY, "drake/objects/block_for_pick_and_place_mid_size.urdf"
)
...
def set_aabb_buffer(buffer=0.0):
...
def set_continuous_collision_penetration(penetration=0.0):
...
def set_continuous_collision_radius(body, link, radius=0.0):
    # https://github.com/bulletphysics/bullet3/blob/0e124cb2f103c40de4afac6c100b7e8e1f5d9e15/examples/pybullet/examples/experimentalCcdSphereRadius.py
...
def synchronize_viewer():
    ...
    p.configureDebugVisualizer(
        p.COV_ENABLE_SINGLE_STEP_RENDERING, True, physicsClientId=CLIENT
    )
...
def connect(
    ...
    with HideOutput():
        ...
        if color is not None:
            options.append(
                "--background_color_red={} --background_color_green={} --background_color_blue={}".format(
                    *color
                )
            )
...
def set_real_time(enable=True):
    p.setRealTimeSimulation(
        enableRealTimeSimulation=int(enable), physicsClientId=CLIENT
    )
...
def get_camera_pose():
    ...
    view_matrix = compute_view_matrix(
        target_point, dist, yaw, pitch, roll=0.0, z_up=True
    )
    # get_ray_from_to
...
def compute_view_matrix(target_position, distance, yaw, pitch, roll=0.0, z_up=True):
...
def compute_camera_pose(camera_point, target_point=np.zeros(3)):
    ...
    # print(camera_pose)
    view_matrix = compute_view_matrix(
        target_point, distance, yaw, pitch, roll=0.0, z_up=True
    )
    return pose_from_tform(view_matrix)
...
def get_image(
    ...
    **kwargs,
    ...
    width, height, rgb, d, seg = p.getCameraImage(
        width,
        height,
...
def dump_joint(body, joint):
    print(
        "Joint id: {} | Name: {} | Type: {} | Circular: {} | Lower: {:.3f} | Upper: {:.3f}".format(
            joint,
...
def dump_body(body, fixed=False, links=True):
    ...
    print(
        "Link id: {} | Name: {} | Mass: {} | Collision: {} | Visual: {}".format(
            ...
            get_base_name(body),
            get_mass(body),
            len(get_collision_data(body, base_link)),
            num_visual,
        )
    )
    for link in get_links(body):
...
def get_adjacent_fixed_links(body):
    return list(
        filter(
            lambda item: not is_movable(body, parent_joint_from_link(item[0])),
...
def assign_link_colors(body, max_colors=3, alpha=1.0, s=0.5, **kwargs):
    # TODO: graph coloring
    # TODO: use depth in tree of components
    components = sorted(
        map(list, get_rigid_clusters(body)),
        key=np.average,
        # key=min,
...
def get_dynamics_info(body, link=BASE_LINK):
    return DynamicsInfo(
        *p.getDynamicsInfo(body, link, physicsClientId=CLIENT)[
            : len(DynamicsInfo._fields)
        ]
    )
...
def set_collision_margin(body, link=BASE_LINK, margin=0.0):
...
def get_faces_geometry(mesh, vertex_textures=None, vertex_normals=None, scale=1.0):
...
Shape = named_tuple(
    "Shape", *unzip([("geom", None), ("pose", Pose()), ("color", None)])
)
...
LinkInfo = named_tuple(
    "LinkInfo",
    *unzip(
        [
            ("mass", STATIC_MASS),
            ("collision_id", NULL_ID),
            ("visual_id", NULL_ID),
            ("point", unit_point()),
            ("quat", unit_quat()),
            ("inertial_point", unit_point()),
            ("inertial_quat", unit_quat()),
            ("parent", 0),
            ("joint_type", p.JOINT_FIXED),
            ("joint_axis", unit_point()),
        ]
    ),
)
...
def create_capsule(radius, height, mass=STATIC_MASS, color=BLUE, **kwargs):
    collision_id, visual_id = create_shape(
        get_capsule_geometry(radius, height), color=color, **kwargs
    )
    return create_body(collision_id, visual_id, mass=mass)
...
def create_sphere(radius, mass=STATIC_MASS, color=BLUE, **kwargs):
    collision_id, visual_id = create_shape(
        get_sphere_geometry(radius), color=color, **kwargs
    )
    return create_body(collision_id, visual_id, mass=mass)
...
def create_plane(normal=[0, 0, 1], mass=STATIC_MASS, color=BLACK, **kwargs):
    ...
    collision_id, visual_id = create_shape(
        get_plane_geometry(normal), color=color, **kwargs
    )
...
def create_faces(
    mesh, scale=1.0, mass=STATIC_MASS, collision=True, color=GREY, **kwargs
):
    # TODO: rename to create_mesh
    collision_id, visual_id = create_shape(
        get_faces_geometry(mesh, scale=scale, **kwargs),
        collision=collision,
        color=color,
    )
    body = create_body(collision_id, visual_id, mass=mass)
...
def get_visual_data(body, link=BASE_LINK):
    ...
    visual_data = [
        VisualShapeData(*tup)
        for tup in p.getVisualShapeData(body, flags, physicsClientId=CLIENT)
    ]
    # return visual_data
...
def clone_body(body, links=None, collision=True, visual=True, client=None):
    ...
    for i, link in enumerate(links):
        ...
        collision_shapes.append(
            clone_collision_shape(body, link, client) if collision else NULL_ID
        )
        visual_shapes.append(
            clone_visual_shape(body, link, client) if visual else NULL_ID
        )
...
def get_mesh_data(obj, link=BASE_LINK, shape_index=0, visual=True):
    ...
    return Mesh(
        *p.getMeshData(obj, linkIndex=link, flags=flags, physicsClientId=CLIENT)
    )
...
def is_aabb_degenerate(aabb):
    return get_aabb_volume(aabb) <= 0.0
...
def get_subtree_aabb(body, root_link=BASE_LINK, **kwargs):
    return aabb_union(
        get_aabbs(body, links=get_link_subtree(body, root_link), **kwargs)
    )
...
def vertices_from_body(body, base_pose=None, **kwargs):
    ...
    for link in get_all_links(body):
        ...
        vertices.extend(
            tform_points(link_pose, vertices_from_link(body, link=link, **kwargs))
        )
    return vertices
...
def set_collision_pair_mask(
    body1, body2, link1=BASE_LINK, link2=BASE_LINK, enable=True
):
...
def get_unbuffered_aabb(body, **kwargs):
    return get_buffered_aabb(body, max_distance=-DEFAULT_AABB_BUFFER / 2.0, **kwargs)
...
def get_contact_points(**kwargs):
    return [
        CollisionInfo(*info)
        for info in p.getContactPoints(physicsClientId=CLIENT, **kwargs)
    ]
...
def get_ray_from_to(mouseX, mouseY, farPlane=10000):
    ...
    rayTo = (
        rayFrom
        + rayForward
        - 0.5 * (np.array(horizon) - np.array(vertical))
        + (mouseX * dHor - mouseY * dVer)
    )
...
def get_refine_fn(body, joints, num_steps=0):
    ...
    def fn(q1, q2):
        ...
        for i in range(num_steps):
            ...
            # q = tuple(positions)
            q = tuple(
                wrap_positions(body, joints, positions)
            )  # TODO: possible issue with adjust path
            yield q
...
def get_moving_pairs(body, moving_joints):
    ...
    moving_links = list(
        filter(
            lambda link: can_collide(body, link), get_moving_links(body, moving_joints)
        )
    )
...
def get_self_link_pairs(body, joints, disabled_collisions=set(), only_moving=True):
    moving_links = list(
        filter(lambda link: can_collide(body, link), get_moving_links(body, joints))
    )
    fixed_links = list(
        filter(
            lambda link: can_collide(body, link),
            set(get_links(body)) - set(moving_links),
        )
    )
...
def get_collision_fn(
    ...
    **kwargs,
    ...
    moving_links = frozenset(
        link for link in get_moving_links(body, joints) if can_collide(body, link)
...
def get_closest_angle_fn(
    body, joints, weights=None, reversible=True, linear_tol=0.0, **kwargs
):
    assert len(joints) == 3
    weights = get_default_weights(body, joints, weights)
    ...
    def closest_fn(q1, q2):
        ...
        for direction in [0, PI] if reversible else [0]:  # TODO: previously was [PI]
            ...
            distance = (
                angular_distance_fn(q1[2:], [angle])
                + linear_distance
...
def get_nonholonomic_extend_fn(
    body, joints, resolutions=None, angular_tol=0.0, **kwargs
):
    assert len(joints) == 3
    resolutions = get_default_resolutions(body, joints, resolutions)
    ...
    def extend_fn(q1, q2):
        ...
        if angle is None:
            return [
                np.append(q1[:2], aq) for aq in angular_extend_fn(q1[2:], q2[2:])
            ]  # TODO: average?
        path = []
        if abs(circular_difference(angle, q1[2])) >= angular_tol:
            path.extend(
                np.append(q1[:2], aq) for aq in angular_extend_fn(q1[2:], [angle])
            )
        path.extend(np.append(lq, [angle]) for lq in linear_extend_fn(q1[:2], q2[:2]))
        if abs(circular_difference(q2[2], angle)) >= angular_tol:
            path.extend(
                np.append(q2[:2], aq) for aq in angular_extend_fn([angle], q2[2:])
            )
        return path
...
def get_dubins_extend_fn(
    body, joints, turning_radius=1e-3, step_size=0.01, **kwargs  # meters  # meters
):
    assert len(joints) == 3
    import dubins
...
def shortcut_circular(body, joints, path, extend_fn, collision_fn, **kwargs):
    ...
    return (
        list(flatten(extend_fn(*pair) for pair in get_pairs(shortcutted)))
        + shortcutted[-1:]
    )
...
def plan_nonholonomic_motion(
    ...
    **kwargs,
    ...
    path = solve(
        ...
        algorithm=algorithm,
        **kwargs,
    )  # weights=weights, # TODO: deliberately excluding for PRM unless circular
    # return path
...
def plan_base_joint_motion(
    ...
    **kwargs,
    ...
    return plan_nonholonomic_motion(
        robot,
        joints,
        goal_positions,
        reversible=reversible,
...
def get_max_accelerations(robot, joints, max_velocities=None, duration_to_max=1.0):
...
def get_dynamical_limits(
    robot, joints, max_velocities=None, max_accelerations=None, duration_to_max=1.0
):
    if max_velocities is None:
    ...
    if max_accelerations is None:
        max_accelerations = get_max_accelerations(
            robot, joints, max_velocities, duration_to_max
        )
...
def get_acceleration_fn(
    ...
    **kwargs,
    ...
    max_velocities, max_accelerations = get_dynamical_limits(
        robot, joints, max_velocities, max_accelerations, duration_to_max=1.0
    )
    return get_duration_fn(
        get_difference_fn(robot, joints),
        v_max=max_velocities,
        a_max=max_accelerations,
        **kwargs,
    )
...
def smooth_path(
    ...
    **kwargs,
    ...
    curve = smooth_cubic(
        path, collision_fn, resolutions, max_velocities, max_accelerations, **kwargs
    )
    return curve
...
def sample_sphere_surface(d, uniform=True):
    ...
    while True:
        v = np.random.randn(d)
        r = np.sqrt(v.dot(v))
        if not uniform or (r <= 1.0):  # TODO: shouldn't need this check?
            return v / r
...
def control_joint(body, joint, position=None, velocity=0.0, **kwargs):
    if position is None:
...
def velocity_control_joint(body, joint, velocity=0.0, **kwargs):
    joint_kwargs = get_control_joint_kwargs(body, joint, **kwargs)
    return p.setJointMotorControl2(
        body,
        joint,
...
def waypoint_joint_controller(
    body, joints, target, tolerance=1e-3, time_step=0.1, timeout=INF, **kwargs
):
    ...
    while time_elapsed < timeout:  # TODO: timeout based on the distance
        ...
        waypoint = convex_combination(
            positions, target, w=w
        )  # Important to not include wrap around
...
def compute_jacobian(
    ...
    translate, rotate = p.calculateJacobian(
        robot, link, point, positions, velocities, accelerations, physicsClientId=CLIENT
    )
    ...
    for target_pose in waypoint_poses:
        ...
        for iteration in irange(max_iterations):
            ...
            sub_kinematic_conf = inverse_kinematics_helper(
                sub_robot, sub_target_link, target_pose, null_space=null_space
            )
            ...
            if is_pose_close(
                get_link_pose(sub_robot, sub_target_link), target_pose, **kwargs
            ):
                set_joint_positions(robot, selected_joints, sub_kinematic_conf)
...
def add_parameter(name, lower=0.0, upper=1.0, initial=0.0):
...
def add_body_name(body, name=None, link=BASE_LINK, **kwargs):
    ...
    return add_text(
        name, position=position, parent=body, parent_link=link, **kwargs
    )  # removeUserDebugItem
...
def draw_link_name(body, link=BASE_LINK, position=Point(y=0.2)):
    return add_text(
        get_link_name(body, link), position=position, parent=body, parent_link=link
    )
...
def draw_global_system(length=1.0, **kwargs):
    return draw_pose(Pose(), length=length, **kwargs)


def draw_pose2d(pose2d, z=0.0, d=2, **kwargs):
...
def is_mesh_on_surface(
    ...
    return (abs(min_z) < epsilon) and all(
        is_point_in_polygon(point, polygon) for point in points_surface
    )
...
def grow_polygon(points, radius=0.0, n=8):
